...
exports.countOrders = async (where, params) => {
  const [result] = await db.query(`SELECT COUNT(*) as total FROM orders o ${where}`, params);
...
exports.addTracking = async (orderId, status, note) => {
  await db.query("INSERT INTO order_tracking (order_id, status, note) VALUES (?, ?, ?)", [orderId, status, note]);
};

exports.addNotification = async (userId, title, message) => {
  await db.query("INSERT INTO notifications (user_id, title, message, type) VALUES (?, ?, ?, 'order')", [userId, title, message]);
};

exports.getStats = async (storeId) => {
  const [stats] = await db.query("SELECT status, COUNT(*) as count FROM orders WHERE store_id = ? GROUP BY status", [storeId]);
  return stats;
};
