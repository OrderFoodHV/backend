...
exports.getProfile = async (userId) => {
  ...
  const [stores] = await db.query("SELECT id, name, address, phone, status, rating, rating_count FROM stores WHERE owner_id = ?", [userId]);
  const [shippers] = await db.query("SELECT status, phone, vehicle, rating, rating_count FROM shippers WHERE user_id = ?", [userId]);
  ...
  user.storeRating = stores.length > 0 ? stores[0].rating : null;
  user.storeRatingCount = stores.length > 0 ? stores[0].rating_count : null;
  ...
  user.shipperRating = shippers.length > 0 ? shippers[0].rating : null;
  user.shipperRatingCount = shippers.length > 0 ? shippers[0].rating_count : null;
