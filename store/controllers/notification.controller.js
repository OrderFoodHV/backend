...
exports.streamNewOrders = async (req, res, next) => {
  try {
    ...
    const interval = setInterval(async () => {
      try {
        const result = await notificationService.pollNewOrders(storeId, lastOrderId);
...
exports.getRecentOrders = async (req, res, next) => {
  try {
    const orders = await notificationService.getRecentOrders(req.params.storeId, req.query.minutes);
