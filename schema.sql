...
-- Phiên bản chuẩn hóa — xóa trùng lặp, đồng bộ với code
-- =====================================================
...
CREATE TABLE users (
    ...
    -- 'user' khớp với code (auth.controller.js default 'user')
...
-- Bảng reviews (chỉ khai báo 1 lần — phiên bản đầy đủ)
...
CREATE INDEX idx_products_store ON products (store_id, available);
...
-- Admin user (password được hash bởi script migrate.js)
-- Xem scripts/migrate.js để tạo với bcrypt

-- Cửa hàng mẫu
INSERT INTO stores (name, address, owner_id) VALUES ('Food App Store', 'Hà Nội', 1);

-- Danh mục
INSERT INTO categories (id, name) VALUES (1, 'Snacks'), (2, 'Fast Food'), (3, 'Drinks');

-- Phí dịch vụ mẫu
INSERT INTO fee_settings (fee_type, fee_value, fee_description, status) VALUES
    ('service_fee',  10.00, 'Phí dịch vụ platform (10%)', 'active'),
    ('shipping_fee', 15.00, 'Phí vận chuyển mặc định',    'active');

-- Voucher hệ thống mẫu
INSERT INTO system_vouchers (code, discount_type, discount_value, min_order_amount, max_discount, quantity, start_date, end_date, status) VALUES
    ('WELCOME10', 'percent', 10.00, 50000, 20000, 100, NOW(), DATE_ADD(NOW(), INTERVAL 30 DAY), 'active'),
    ('FREESHIP',  'fixed',   15000, 100000, 15000, 50, NOW(), DATE_ADD(NOW(), INTERVAL 7 DAY),  'active');

-- Voucher người dùng mẫu
INSERT INTO vouchers (code, discount_percent, discount_amount, min_order_amount, max_uses, expired_at) VALUES
    ('WELCOME10', 10, 0,     100000, 100, '2025-12-31 23:59:59'),
    ('FOODIE20',  20, 0,     200000,  50, '2025-12-31 23:59:59'),
    ('SAVE50K',    0, 50000, 300000,  30, '2025-12-31 23:59:59'),
    ('FREESHIP',   0, 30000, 150000, 200, '2025-12-31 23:59:59');
